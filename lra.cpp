...
void setup(){
  ...
  w(0x16, 0x4F);
  w(0x17, 0x80);     // OD_CLAMP raised
  w(0x1A, 0xB6);     // LRA mode
  w(0x1B, 0x9A);     // 160 Hz drive time
  w(0x1C, 0xF5);
  w(0x1D, 0xA9);     // open-loop LRA
  w(0x20, 0x3F);     // 160 Hz period
  
  w(0x02, 0xA0);     // proven safe-strong amplitude (level 3 in ramp test)

  Serial.println("Continuous strong pulses: 500ms ON / 500ms OFF.");
  Serial.println("Watching VBAT for safety.");
}
